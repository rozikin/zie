...
const ProductImage: React.FC<ProductImageProps> = ({
  ...
  return (
    <div className='grid grid-cols-6 gap-2 h-full max-h-[500px] min-h-[300px] sm:min-h-[400px]'>
        <div className='flex flex-col tems-center justify-center gap-4 cursor-pointer border h-full max-h-[500px] min-h-[300px] sm:min-h-[400px] '>
            {product.images.map((image: selectImgType) =>{
                ...
                ${cartProduct.selectedImg.color === image.color? 'border-[1.5px]' :'border-none'}`}>
                    <img src={image.image} alt={image.color} className='object-fill' />
                </div>
            })}
        </div>
        <div></div>
